...
public class ExpressionTree {
	...
	public Node buildExpTree(String s, int x, int y) {
		...
		if (BoundProcessor.isVariable(sub) || BoundProcessor.isDouble(sub)) // 利用工具类Utils的相关方法，利用正则表达式匹配s[x,y)内容
		{
			if (assignMode && BoundProcessor.isVariable(sub)
					&& !BoundProcessor.getAssignMap()[BoundProcessor.getVariableID(sub)].equals(""))
				return new Node(BoundProcessor.getAssignMap()[BoundProcessor.getVariableID(sub)], null, null); // 构造叶子节点，赋值状态，如果有变量被赋值，替换成该值
			else
